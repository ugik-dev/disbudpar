...
document.getElementById("export_btn").href = '<?= site_url('AdminController/PdfSenibudaya?id_senibudaya=')?>'+id_senibudaya;
var Photo2Modal = {
    'self': $('#photo2_modal'),
...
function renderPhotoModal(){
  ...
  function MessageFunction() {
    ...
    formatMessage = `Pada Seni Budaya - `+dataProfil['nama']+`
`;
...
document.getElementById("approv_profil_btn").onclick = function() {ApprovProfil()};
  ...
  function ApprovProfil() {
    swal(swalApprovConfigure).then((result) => {
      ...
      $.ajax({
        url: `<?=site_url('DetailSenibudayaController/approv')?>`, 'type': 'get',
...
/* myFunction toggles between adding and removing the show class, which is used to hide and show the dropdown content */
  ...
  $("#tahun_input").click(function(e) {
    if(dataProfil['id_user_approv']=='0'){
      ...
      swal("Data Belum di Approv",'Harap Konformasi ke Admin Untuk Approval', "error");
    }else{
